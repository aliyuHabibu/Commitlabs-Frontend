...
export function parseBoundedPagination(searchParams: URLSearchParams): {
  page: number;
  pageSize: number;
} {
  ...
  if (pageSize > MARKETPLACE_MAX_PAGE_SIZE)
    throw new ValidationError(
      `Invalid 'pageSize' query param. Maximum allowed is ${MARKETPLACE_MAX_PAGE_SIZE}.`,
    );
  if (page > MARKETPLACE_MAX_PAGE)
    throw new ValidationError(
      `Invalid 'page' query param. Maximum allowed is ${MARKETPLACE_MAX_PAGE}.`,
    );
...
export function validateListingId(raw: string): string {
  ...
  if (id.length > MARKETPLACE_LISTING_ID_MAX_LENGTH)
    throw new ValidationError(
      `Listing ID must be at most ${MARKETPLACE_LISTING_ID_MAX_LENGTH} characters.`,
    );
  if (!LISTING_ID_PATTERN.test(id))
    throw new ValidationError(
      'Listing ID contains invalid characters. Only letters, digits, hyphens, and underscores are allowed.',
    );
  return id;
}
